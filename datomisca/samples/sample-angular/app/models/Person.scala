...
object Person {
  ...
  implicit val partialUpdate: Reads[PartialAddEntity] = (
    ((__ \ 'name).read(readAttr[String](Person.name)) orElse Reads.pure(PartialAddEntity.empty)) and
    ((__ \ 'age) .read(readAttr[Long](Person.age)) orElse Reads.pure(PartialAddEntity.empty))  and
    // need to specify type because a ref/many can be a list of dref or entities so need to tell it explicitly
    ((__ \ 'characters).read( readAttr[Set[DRef]](Person.characters) ) orElse Reads.pure(PartialAddEntity.empty))
    reduce
  )
